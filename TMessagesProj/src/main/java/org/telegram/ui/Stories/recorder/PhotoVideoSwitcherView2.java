...
public class PhotoVideoSwitcherView2 extends View implements FlashViews.Invertable {

    private TextPaint textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
    private Paint selectorPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    public StaticLayout photoText, videoText;
    ...
    private float targetTextRotation = 0f;
    private ValueAnimator rotationAnimator;

    public PhotoVideoSwitcherView2(Context context) {
        super(context);
...
//        float insetDp = dp(6);
//        selectorRect.inset(insetDp, 0);
        canvas.save();
        float selectorCenterX = selectorRect.centerX();
        float selectorCenterY = selectorRect.centerY();
        canvas.translate(selectorCenterX, selectorCenterY);
        ...
        canvas.rotate(currentTextRotation, photoTextWidth / 2f, photoTextHeight / 2f);
        photoText.draw(canvas);
        canvas.restore();

        canvas.save();
        canvas.translate(x + dp(4 + 12) - videoTextLeft, cy - videoTextHeight / 2f + oy);
        canvas.rotate(currentTextRotation, videoTextWidth / 2f, videoTextHeight / 2f);
    ...
    public void setTextRotation(float angle) {
        ...
        rotationAnimator = ValueAnimator.ofFloat(currentTextRotation, angle);
        rotationAnimator.addUpdateListener(animation -> {
            currentTextRotation = (float) animation.getAnimatedValue();
            invalidate();
        });
        rotationAnimator.setDuration(200);
        rotationAnimator.setInterpolator(new LinearInterpolator());
        rotationAnimator.start();
        targetTextRotation = angle;
    }
