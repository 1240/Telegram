...
public class ProfileStoriesView extends View implements NotificationCenter.NotificationCenterDelegate {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        // Компенсируем смещение, возникающее из‑за масштабирования вокруг pivot.
        float pivotX = avatarContainer.getPivotX();
        float pivotY = avatarContainer.getPivotY();

        float ax = avatarContainer.getX() - pivotX * (avatarContainer.getScaleX() - 1f) + insetMain;
        float ay = avatarContainer.getY() - pivotY * (avatarContainer.getScaleY() - 1f) + insetMain;

        float aw = avatarContainer.getWidth() * avatarContainer.getScaleX() - insetMain * 2;
        float ah = avatarContainer.getHeight() * avatarContainer.getScaleY() - insetMain * 2;
