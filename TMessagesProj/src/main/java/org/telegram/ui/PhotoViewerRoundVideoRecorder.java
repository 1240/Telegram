...
public class PhotoViewerRoundVideoRecorder extends FrameLayout {
    ...
    public PhotoViewerRoundVideoRecorder(Context context) {
        super(context);
        ...
        cameraView = new CameraView(context, true, false) {
            ...
            protected void receivedAmplitude(double amplitude) {
                PhotoViewerRoundVideoRecorder.this.receivedAmplitude(amplitude);
            }
        };
    ...
    public PhotoViewerRoundVideoRecorder onDone(Utilities.Callback3<File, String, Long> onDoneCallback) {
        this.onDoneCallback = onDoneCallback;
        return this;
    }

    private Runnable onDestroyCallback;
    public PhotoViewerRoundVideoRecorder onDestroy(Runnable onDestroyCallback) {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        Rect r1 = new Rect();
        cameraView.getGlobalVisibleRect(r1);
        AndroidUtilities.rectTmp.set(
                r1.left + cameraView.getWidth() / 2f * (1f - cameraView.getScaleX()),
                r1.top + cameraView.getHeight() / 2f * (1f - cameraView.getScaleY()),
                r1.left + cameraView.getWidth() - cameraView.getWidth() / 2f * (1f - cameraView.getScaleX()),
                r1.top + cameraView.getHeight() - cameraView.getHeight() / 2f * (1f - cameraView.getScaleY())
        );
    ...
    public void hideTo(RoundView roundView) {
        ...
        post(() -> {
            ...
            Rect r1 = new Rect();
            roundView.getGlobalVisibleRect(r1);
            Rect r2 = new Rect();
            cameraView.getGlobalVisibleRect(r2);
            final float toX = (r1.left + roundView.getWidth() / 2f) - (r2.left + cameraView.getWidth() / 2f);
            final float toY = (r1.top + roundView.getHeight() / 2f) - (r2.top + cameraView.getHeight() / 2f);
            ...
            cameraViewAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if (getParent() instanceof ViewGroup) {
                        ((ViewGroup) getParent()).removeView(PhotoViewerRoundVideoRecorder.this);
                    }
                }
            });
